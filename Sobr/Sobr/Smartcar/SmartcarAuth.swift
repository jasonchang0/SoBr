...
@objc public class SmartcarAuth: NSObject {
    ...
    @objc public func unlockVehicle() {
        ...
        let url = URL(string: "https://api.smartcar.com/v1.0/vehicles")!
        ...
        let task = session.dataTask(with: request as URLRequest, completionHandler: { data, response, error in
            ...
            do {
                ...
                if let json = try JSONSerialization.jsonObject(with: data, options: .mutableContainers) as? [String: Any] {
                    let vehicles: [String] = (json["vehicles"]) as! [String]
                    let vehicle: String = vehicles[0]

                    var request = URLRequest(url: URL(string: "https://api.smartcar.com/v1.0/vehicles/\(vehicle)/security")!)
                    request.httpMethod = "POST" //set http method as POST
                    request.httpBody = "{\"action\": \"LOCK\"}".data(using: String.Encoding.utf8)
                    ...
                    let task1 = session.dataTask(with: request as URLRequest, completionHandler: { data, response, error in
                        ...
                        do {
                            //create json object from data
                            if let json = try JSONSerialization.jsonObject(with: data, options: .mutableContainers) as? [String: Any] {
                                print(json["status"] ?? "status is nil")
                            }
                        } catch let error {
